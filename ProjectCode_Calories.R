...
# based on this, we are not dealing with MCAR missing data. To make sure of our insight we run a hypothesis test to 
...
# Imputation methods analysis - Pooling
# For the models, we only analyze pmm and sample since based on the diagnostic plots
# norm is not  a good method since shows some data that are not plausible
...
summary(lm(calories ~ calhour, data = data.ipw, weights = data.ipw$w))
confint(data.ipw.results)
...
# AIC - PMM
data.pmm <- complete(data_pmm)
pmm.AIC <- step(lm(calories ~ 1, data = data.pmm), scope = ~ weight + calhour, direction = "forward")

# AIC - IPW
ipw.AIC <- step(lm(calories ~ 1, data = data.ipw, weights = data.ipw$w), scope = ~ weight + calhour + ,
                direction = "forward")

# Based on AIC criterion - we select IPW as our final model!

# Final model - IPW
summary(data.ipw.results)
# Confidence interval
confint(data.ipw.results, level = 0.95)
# Plots and correlation
data.real <- read.table("muscle-incomplete.txt", header = TRUE)
cor(x=data.real, use = "complete.obs")

data.real$weight <- as.factor(data.real$weight)
ggplot(data.real, aes(x=calhour, y = calories, color = weight)) + geom_point(size = 3) + geom_smooth(method=lm, se = F) + xlab("Work levels [calories/hour]") + ylab("Heat production [calories]") 
